...
Note: tools that were never in the *old* registry (e.g. tailwindcss, vuejs, ollama, wireshark) show score 0 / 0 stars until the **next scrape** runs — they're now correctly registered, so one cycle (needs `GITHUB_TOKEN` for reliable rates) populates them.
