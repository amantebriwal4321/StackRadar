...
class Domain(Base):
    ...
    tools = relationship(
        "Tool", back_populates="domain_rel", cascade="all, delete-orphan"
    )
...
class Tool(Base):
    ...
    parent_tool = relationship(
        "Tool", remote_side="Tool.id", backref="children", foreign_keys=[parent_tool_id]
    )
    ...
    snapshots = relationship(
        "ToolSnapshot", back_populates="tool", cascade="all, delete-orphan"
    )
...
class UserProgress(Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            "user_id", "roadmap_slug", "step", name="uq_user_roadmap_step"
        ),
    )
