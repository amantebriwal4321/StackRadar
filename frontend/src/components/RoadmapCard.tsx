...
export default function RoadmapCard({ roadmap, domain, index }: RoadmapCardProps) {
  // Derive difficulty range from steps
  const levels = Array.from(new Set(roadmap.steps.map(s => s.level)));
  const difficultyText = levels.length > 1 
    ? `${levels[0]} to ${levels[levels.length - 1]}`
    : levels[0];

  return (
    <MotionLink 
      href={`/roadmap/${roadmap.technologyId}`}
        ...
        <div className="relative z-10 flex flex-col h-full">
          ...
          <div className="space-y-3 pt-4 border-t border-border/40">
            <div className="flex items-center text-xs text-muted-foreground font-medium">
              ...
              <span>{roadmap.steps.length} Steps ({difficultyText})</span>
            </div>
