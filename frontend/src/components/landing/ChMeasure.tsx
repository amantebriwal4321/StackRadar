          ...
          <ol className="space-y-6">
            {sources.map((s, i) => (
              ...
              >
                ...
                />
                <h3 className="mt-5 text-[24px] font-medium tracking-[-0.02em] text-[var(--c-ink)]">
                  {s.name}
                </h3>
                <p className="mt-2 max-w-[38ch] text-[15px] font-medium leading-relaxed text-[var(--c-ink-2)]">
                  {s.detail}
                </p>
                <p className="mt-6 flex items-baseline gap-2">
                  <span
                    className="font-mono text-[clamp(1.75rem,3vw,2.5rem)] font-medium tabular-nums tracking-[-0.03em] text-[var(--c-ink)]"
                    data-sc-count={`0 ${s.figure}`}
                    data-sc-count-at="0.1 0.5"
                  >
                    {s.figure}
                  </span>
                  <span className="text-[13px] font-medium text-[var(--c-ink-3)]">
                    {s.unit}
                  </span>
                </p>
              </li>
