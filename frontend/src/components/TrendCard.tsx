...
function GrowthIndicator({ growth }: { growth: number }) {
  if (growth > 0) {
    return (
      <span className="inline-flex items-center gap-1 text-emerald-500 dark:text-emerald-600 font-semibold text-xs">
    ...
    return (
      <span className="inline-flex items-center gap-1 text-rose-500 dark:text-rose-600 font-semibold text-xs">
...
export default function TrendCard({ tool, variant = "default", index = 0 }: ToolCardProps) {
    ...
    return (
      ...
      >
        ...
        <div className="mt-2 flex items-end justify-between">
          <div>
            <p className="text-[10px] text-muted-foreground uppercase font-semibold tracking-wider mb-0.5">Score</p>
            <p className="text-3xl font-normal text-foreground">{tool.score}</p>
          </div>
          <div className="text-right space-y-1">
            <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded text-[10px] font-semibold border ${stageBadge[tool.stage] || stageBadge["Emerging"]}`}>
              {tool.stage}
            </span>
  ...
  return (
    <MotionLink
      ...
      whileHover={{ y: -5, scale: 1.02 }}
      transition={{ type: "spring", stiffness: 400, damping: 25 }}
      className="group relative block p-6 bg-card rounded-2xl border border-border/60 glow-hover transition-colors duration-300 hover:border-primary/40 slide-up"
    ...
    >
      ...
      <div className="flex justify-between items-center mb-4 relative z-10 pr-8">
        <div className="flex items-center gap-4 min-w-0">
          ...
          <div className="min-w-0">
            ...
            <div className="flex gap-1.5 flex-wrap mt-1">
              <span className={`px-2 py-0.5 rounded-md text-[10px] font-semibold border ${stageBadge[tool.stage] || stageBadge["Emerging"]}`}>
                {tool.stage}
              </span>
              <span className={`px-2 py-0.5 rounded-md text-[10px] font-semibold border ${priorityBadge[tool.learning_priority] || priorityBadge["MEDIUM"]}`}>
                {tool.learning_priority === "HIGH" ? "🔥 HIGH" :
                 tool.learning_priority === "AVOID" ? "⚠️ CAUTION" :
                 tool.learning_priority}
              </span>
              {sentiment.label && (
                <span className={`px-2 py-0.5 rounded-md text-[10px] font-semibold border ${sentiment.class} hidden sm:inline-block`}>
                  {sentiment.label}
                </span>
              )}
            </div>
        ...
        <div className="text-right shrink-0">
          <span className="block text-[10px] text-muted-foreground uppercase font-semibold tracking-wider mb-0.5">
            Score
          </span>
          <span className="text-2xl font-normal text-foreground drop-shadow-sm">
            {tool.score}
          </span>
          <div className="mt-1">
            <GrowthIndicator growth={tool.growth_pct} />
        ...
        {tool.rank_in_category > 0 && (
          <div className="flex items-center justify-between mb-2 pb-2 border-b border-border/20 text-xs">
            <span className="text-muted-foreground">
              <span className="font-semibold text-foreground">#{tool.rank_in_category}</span> in {tool.category}
            </span>
            <span className="text-muted-foreground">
              Top <span className="font-semibold text-foreground">{tool.percentile}%</span> overall
            </span>
          </div>
        )}
        <div className="grid grid-cols-3 gap-1 text-center text-xs">
          <div className="hover:bg-muted/40 rounded-lg py-1 transition-colors">
            <div className="font-semibold text-foreground text-sm">{tool.stars >= 1000 ? `${(tool.stars / 1000).toFixed(0)}k` : tool.stars}</div>
            <div className="text-muted-foreground text-[10px] uppercase font-semibold">Stars</div>
          </div>
          <div className="hover:bg-muted/40 rounded-lg py-1 transition-colors">
            <div className="font-semibold text-foreground text-sm">{tool.forks >= 1000 ? `${(tool.forks / 1000).toFixed(0)}k` : tool.forks}</div>
            <div className="text-muted-foreground text-[10px] uppercase font-semibold">Forks</div>
          </div>
          <div className="hover:bg-muted/40 rounded-lg py-1 transition-colors">
            <div className="font-semibold text-foreground text-sm">{totalMentions}</div>
      ...
      <div className="mt-4 flex items-center text-sm font-semibold text-primary transition-all relative z-10">
