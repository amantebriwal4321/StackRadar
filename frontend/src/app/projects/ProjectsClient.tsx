...
import { type ProjectSummary, type ProjectTier } from "@/data/trends";
...
export default function ProjectsClient({ projects }: { projects: ProjectSummary[] }) {
  ...
  /* Server-rendered, so there is no loading state to be in — the list is
     either here or it is not. An empty list means the API could not be reached
     when this page was last built or revalidated, which the copy states
     plainly rather than spinning forever. */
  const failed = projects.length === 0;
        ...
        <header className="p-6 md:p-8 rounded-2xl border border-indigo-500/10 bg-[var(--c-surface)]/80 backdrop-blur-md space-y-3">
          <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-6">
            ...
            <div className="flex items-center gap-6 font-mono">
              {[
                { v: failed ? "—" : String(shown.length), l: "projects" },
                { v: failed ? "—" : `${totalHours}h`, l: "to build" },
              ].map((s) => (
